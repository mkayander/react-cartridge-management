...
class CustomChat extends Component {
    ...
    chatHistoryLoad() {
        if (this.state.messageResponse.length === 0) {
            this.props.data.reverse().forEach((element, index) => {
                ...
                this.messageList.push(this.generate(
                    element.message,
                    index,
                    end_date,
    ...
    closeBtnGenerate() {
        if (this.state.btnLaunchClick) {
            return (<ChatOutlinedIcon fontSize="large" classes={{root: "rcw-customchat-launcher-icon"}}/>);
        } else {
            return (<CloseOutlinedIcon fontSize="large" classes={{root: "rcw-customchat-launcher-icon"}}/>);
        }
    };
    ...
    generate(message, index, date, usOrResp) {
        return (
            <div className="rcw-customchat-message" key={index}>
                <div className={`rcw-customchat-${usOrResp ? "client" : "response"}`}>
    ...
    render() {
        return (
            <React.Fragment>
                <Paper elevation={10} classes={{root: `rcw-customchat-main-div ${this.state.chatHiddenOrActive}`}}>
                    ...
                    <div className="rcw-customchat-list">
                        ...
                        {this.state.newMessageList.map((el, index) => this.generate(el.message, index, el.time.toLocaleString(), el.user === this.state.user))}
                    ...
                    <div className="rcw-customchat-input-div">
                        ...
                        >
                            <SendIcon classes={{root: "rcw-customchat-sender-icon"}}/>
                        </Button>
                    </div>
                </Paper>
                <IconButton
